...
describe("MintNFT", function () {
  ...
  describe("getOwnedTokenURIsByAddress", async () => {
    it("get owned token URIs by address", async () => {
      const { publicInputCalldata: publicInputCalldata2, proofCalldata: proofCalldata2 } = await generateProof();
      ...
      const mintNftTxn = await mintNFT
        .connect(organizer)
        .mintParticipateNFT(createdGroupId, eventsList[0].eventRecordId, proofCalldata2);
      await mintNftTxn.wait();
      
      const ownerTokensDetails = await mintNFT
        .connect(organizer)
        .getOwnerTokensDetails(organizer.address);
      console.log("ownerTokensDetails", ownerTokensDetails);
    });
  });
